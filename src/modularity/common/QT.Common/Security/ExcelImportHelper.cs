...
namespace QT.Common.Helper
{
    ...
    public class ExcelImportHelper
    {
        ...
        /// <param name="HeaderRowIndex">工作表标题行所在行号(从0开始).</param>
        /// <returns></returns>
        public static DataTable ToDataTable(Stream stream, bool isxls = false, int SheetIndex = 0, int HeaderRowIndex = 0)
        {
            IWorkbook workbook = isxls ? new HSSFWorkbook(stream): new XSSFWorkbook(stream);
            string SheetName = workbook.GetSheetName(SheetIndex);
            return ToDataTable(workbook, SheetName, HeaderRowIndex);
        }
