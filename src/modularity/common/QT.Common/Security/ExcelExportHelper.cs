                ...
                foreach (T item in list)
                {
                    ...
                    foreach (ExcelColumnModel column in excelConfig.ColumnModel)
                    {
                        ...
                        if (colunValue != null)
                        {
                            ...
                            //    {
                            //        var pictureIndex = workbook.AddPicture(null, PictureType.PNG);

                            //        // 6. 创建锚点（定位图片位置）
                            //        IClientAnchor anchor = workbook.crea();

                            //        // 设置锚点位置（从B2单元格开始）
                            //        anchor.Col1 = newCell.ColumnIndex;   // 起始列
                            //        anchor.Row1 = newCell.RowIndex;      // 起始行
                            //        anchor.Col2 = newCell.ColumnIndex + 2; // 结束列（向右跨越2列）
                            //        anchor.Row2 = cell.RowIndex + 4;   // 结束行（向下跨越4行）
                ...
                foreach (Dictionary<string, object> item in list)
                {
                    ...
                    foreach (ExcelColumnModel column in excelConfig.ColumnModel)
                    {
                        ...
                        if (colunValue.Key != null && colunValue.Value != null)
                        {
                            ...
                            SetCell(newCell, dateStyle, colunValue.Value.GetType(), drValue);
                            ordinal++;
                        }
                        else
                        {
...
public class ExcelExportHelper
{
    ...
    public static void MergeExcels(string[] excelFilePaths, string desFilePath)
    {
