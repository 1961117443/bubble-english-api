...
public class UserAgentScope : IScoped
{
    private static Parser _parse = Parser.GetDefault();
    ...
    private readonly bool _isMobileBrowser;
    #endregion
    ...
    public UserAgentScope(IHttpContextAccessor httpContextAccessor)
    {
        _userAgent = httpContextAccessor.HttpContext?.Request?.Headers["User-Agent"] ?? "";

        if (_userAgent != null)
        {
            _client = _parse.Parse(_userAgent);

            _isMobileBrowser = b.IsMatch(_userAgent) || v.IsMatch(_userAgent.Substring(0, 4));
        }
    }
    ...
    /// 请求客户端是否为移动端
    /// </summary>
    public bool isMobileBrowser => _isMobileBrowser;
}
