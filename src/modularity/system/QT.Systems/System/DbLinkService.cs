...
public class DbLinkService : IDbLinkService, IDynamicApiController, ITransient
{
    ...
    {
        var list = await _repository.Context.Queryable<DbLinkEntity, UserEntity, UserEntity>(
            (a, b, c) => new JoinQueryInfos(JoinType.Left, a.CreatorUserId == b.Id, JoinType.Left, a.LastModifyUserId == c.Id))
                ...
                WhereIF(input.keyword.IsNotEmptyOrNull(), a => a.FullName.Contains(input.keyword)).OrderBy(a => a.SortCode).OrderBy(a => a.CreatorTime, OrderByType.Desc)
                .Select((a, b, c) => new DbLinkListOutput()
                {
                    ...
                    sortCode = a.SortCode
                }).Distinct().ToPagedListAsync(input.currentPage, input.pageSize);
    ...
    {
        if (await _repository.AnyAsync(x => x.FullName == input.fullName && x.DeleteMark == null))
    ...
    {
        if (await _repository.AnyAsync(x => x.Id != id && x.FullName == input.fullName && x.DeleteMark == null))
            throw Oops.Oh(ErrorCode.COM1004);
        var entity = input.Adapt<DbLinkEntity>();
        if (input.oracleExtend)
        {
    ...
    {
        return await _repository.Context.Queryable<DbLinkEntity, UserEntity, UserEntity, DictionaryDataEntity, DictionaryTypeEntity>(
    ...
    {
        return await _repository.FirstOrDefaultAsync(m => m.Id == id && m.DeleteMark == null);
    }
    #endregion
}