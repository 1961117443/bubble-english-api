...
using QT.Systems.Entitys.Dto.Province;
...
namespace QT.Systems.Core.Province;
...
public class ProvinceService : IDynamicApiController, ITransient
{
    ...
    public async Task<dynamic> GetList(string nodeid, [FromQuery] KeywordInput input)
    {
        ...
        {
            var flag = await _repository.AnyAsync(m => m.ParentId == item.id && m.DeleteMark == null);
            item.isLeaf = !flag;
    ...
    {
        var data = (await _repository.ToListAsync(m => m.ParentId == id && m.DeleteMark == null && m.EnabledMark == 1)).OrderBy(o => o.SortCode).ToList();
        if (!areaId.Equals("0"))
            data.RemoveAll(x => x.Id == areaId);
        var output = data.Adapt<List<ProvinceSelectorOutput>>();
        var pids = output.Select(x => x.id).ToArray();
        var parentCount = await  _repository.Where(x => pids.Contains(x.ParentId) && x.DeleteMark == null)
            .GroupBy(x => x.ParentId)
            .Select(x => new
            {
                x.ParentId,
                Count = SqlFunc.AggregateCount(x.ParentId)
            })
            .ToListAsync();
        foreach (var item in output)
        {
            //item.isLeaf = !await _repository.AnyAsync(m => m.ParentId == item.id && m.DeleteMark == null);
            item.isLeaf = !parentCount.Any(m => m.ParentId == item.id);
        }
    ...
    {
        return (await _repository.FirstOrDefaultAsync(m => m.Id == id && m.DeleteMark == null)).Adapt<ProvinceInfoOutput>();
    }
    ...
    {
        if (!await _repository.AnyAsync(m => m.Id == id && m.DeleteMark == null) || await _repository.AnyAsync(m => m.ParentId == id && m.DeleteMark == null))
            throw Oops.Oh(ErrorCode.D1007);
        var isOk = await _repository.Context.Updateable<ProvinceEntity>().SetColumns(it => new ProvinceEntity()
        {
            DeleteMark = 1,
    ...
    {
        if (await _repository.AnyAsync(x => x.Id != id && x.EnCode == input.enCode && x.DeleteMark == null))
            throw Oops.Oh(ErrorCode.COM1004);
        var entity = input.Adapt<ProvinceEntity>();
