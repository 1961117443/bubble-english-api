...
/// 业务实现：配送路线.
/// </summary>
[ApiDescriptionSettings(ModuleConst.ERP, Tag = "Erp", Name = "ErpDeliveryroute", Order = 200)]
[Route("api/Erp/[controller]")]
public class ErpDeliveryrouteService : IErpDeliveryrouteService, IDynamicApiController, ITransient
{
    /// <summary>
    /// 服务基础仓储.
    /// </summary>
    ...
    public async Task Create([FromBody] ErpDeliveryrouteCrInput input)
    {
        ...
        //    _db.BeginTran();
            ...
            if(erpDeliveryCustomerEntityList != null)
            {
                ...
                await _repository.Context.Insertable(erpDeliveryCustomerEntityList).ExecuteCommandAsync();
            }
            ...
            if(erpDeliveryrouteDeliverymanEntityList != null)
            {
                ...
                await _repository.Context.Insertable(erpDeliveryrouteDeliverymanEntityList).ExecuteCommandAsync();
            }
