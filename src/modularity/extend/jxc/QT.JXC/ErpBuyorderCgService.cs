...
/// 业务实现：采购任务订单.
/// </summary>
[ApiDescriptionSettings(ModuleConst.ERP, Tag = "Erp", Name = "ErpBuyorderCg", Order = 200)]
...
public class ErpBuyorderCgService : IDynamicApiController, ITransient
{
    ...
    public ErpBuyorderCgService(
        ...
        IUserManager userManager, IFileManager fileManager, ICoreSysConfigService coreSysConfigService)
    {
    ...
    public async Task<dynamic> GetInfo(string id)
    {
        ...
        if (config.erpShareStock.IsNotEmptyOrNull())
        {
            ...
            if (companys.IsAny() && companys.Contains(output.oid))
            {
                var gids = output.erpBuyorderdetailList.Select(x => x.gid).ToList();
              ...
              .Where((a, b) => a.Oid != output.oid && companys.Contains(a.Oid) && b.Num > 0)
              .Where((a, b) => gids.Contains(b.Gid))
    ...
    {
        var taskToUserIds = new List<string>();
        if (input.taskToUserName.IsNotEmptyOrNull())
        {
            taskToUserIds = await _repository.Context.Queryable<UserEntity>().Where(d => d.RealName.Contains(input.taskToUserName)).Select(d => d.Id).ToListAsync();

            taskToUserIds.Add(input.taskToUserName);
        }
        ...
        var data = await _repository.Context.Queryable<ErpBuyorderEntity>()
            ...
            //.WhereIF(!string.IsNullOrEmpty(input.taskToUserName), it => SqlFunc.Subqueryable<UserEntity>().Where(d => d.Id == it.TaskToUserId && d.RealName.Contains(input.taskToUserName)).Any())
            .WhereIF(taskToUserIds.Any(), it => SqlFunc.Subqueryable<ErpBuyorderUserEntity>().Where(d => d.Bid == it.Id && taskToUserIds.Contains(d.Uid)).Any())
    ...
    /// <param name="id"></param>
    /// <returns></returns>
    [HttpPost("AdminDetail/Complete/{id}")]
    [SqlSugarUnitOfWork]
    public async Task AdminDetailComplete(string id)
    {
        var entity = await _repository.Context.Queryable<ErpBuyorderdetailEntity>().InSingleAsync(id) ?? throw Oops.Oh(ErrorCode.COM1005);
        ...
        if (!entity.BuyTime.HasValue)
        {
            throw Oops.Oh("订单明细缺少采购时间！");
        }
        if (entity.BuyState == "2")
        {
            throw Oops.Oh("订单明细已完成入库！");
        }
        if (entity.Num <=0)
        {
            throw Oops.Oh("采购数量不能小于0！");
        }
