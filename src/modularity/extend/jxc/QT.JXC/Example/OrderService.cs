    ...
    public class OrderService : IDynamicApiController, ITransient
    {
        ...
        public async Task<dynamic> GetList([FromQuery] OrderListQuery input)
        {
            ...
            if (input.keyword.IsNotEmptyOrNull())
                queryWhere = queryWhere.And(m => m.orderCode.Contains(input.keyword) || m.customerName.Contains(input.keyword) || m.salesmanName.Contains(input.keyword));
            ...
            if (_userManager.User.IsAdministrator == 0)
                authorizeWhere = await _userManager.GetConditionAsync<OrderListOutput>("F4FCA43A-B563-4149-AB16-486589CFC154", "id");
            var pageList = await _repository.Context.Queryable<OrderEntity, UserEntity, FlowTaskEntity>((a, b, c) => new JoinQueryInfos(JoinType.Left, a.CreatorUserId == b.Id, JoinType.Left, a.Id == c.Id))
                .Where((a, b) => a.DeleteMark == null)
                .Select((a, b, c) => new OrderListOutput()
                {
        ...
        {
            var data = await _repository.Context.Queryable<OrderEntryEntity>().Where(x => x.OrderId == id).OrderBy(x => x.SortCode).ToListAsync();
        ...
        {
            var data = await _repository.Context.Queryable<OrderReceivableEntity>().Where(x => x.OrderId == id).OrderBy(o => o.SortCode).ToListAsync();
        ...
        {
            var data = (await _repository.FirstOrDefaultAsync(x => x.Id == id && x.DeleteMark == null)).Adapt<OrderInfoOutput>();
            data.collectionPlanList = (await _repository.Context.Queryable<OrderReceivableEntity>().Where(x => x.OrderId == id).ToListAsync()).Adapt<List<CollectionPlanModel>>();
            data.goodsList = (await _repository.Context.Queryable<OrderEntryEntity>().Where(x => x.OrderId == id).ToListAsync()).Adapt<List<GoodsModel>>();
        ...
        {
            //var kh = _repository.Context.Queryable<ExtErpProducttypeEntity>()
            //    .Select(f => new
            //    {
            //        f.Id,
            //       f.Name
            //    }).ToList();
            //.Where(x => x.n == organizeId && x.ObjectType == "Role" && userRoleIds.Contains(x.ObjectId)).Select(x => x.ObjectId).ToList();
            //kh.ToJsonString();// 
        ...
        {
            //// 当前组织下的角色Id 集合
           // var kh = _repository.Context.Queryable<ExtErpProducttypeEntity>().ToList();
            //.Where(x => x.n == organizeId && x.ObjectType == "Role" && userRoleIds.Contains(x.ObjectId)).Select(x => x.ObjectId).ToList();
            //kh.ToJsonString();// 
            //// 全局角色Id 集合
        ...
        public async Task Create([FromBody] OrderCrInput input)
        {
            ...
            orderEntity.Id = SnowflakeIdHelper.NextId();
            ...
            {
                itemEntity.Id = SnowflakeIdHelper.NextId();
            ...
            await _repository.Context.Insertable(orderEntryList).ExecuteCommandAsync();
            await _repository.Context.Insertable(orderReceivableList).ExecuteCommandAsync();
            await _repository.Context.Insertable(orderEntity).CallEntityMethod(m => m.Create()).ExecuteCommandAsync();
            if (input.status == 0)
            {
                var flowEngineEntity = await _repository.Context.Queryable<FlowEngineEntity>().FirstAsync(a => a.EnCode == "crmOrder" && a.EnabledMark == 1 && a.DeleteMark == null);
        ...
        public async Task Update(string id, [FromBody] OrderUpInput input)
        {
            ...
            await _repository.Context.Deleteable<OrderEntryEntity>(x => x.OrderId == id).ExecuteCommandAsync();
            await _repository.Context.Deleteable<OrderReceivableEntity>(x => x.OrderId == id).ExecuteCommandAsync();
            await _repository.Context.Insertable(orderEntryList).ExecuteCommandAsync();
            await _repository.Context.Insertable(orderReceivableList).ExecuteCommandAsync();
            await _repository.Context.Updateable(orderEntity).IgnoreColumns(ignoreAllNullColumns: true).CallEntityMethod(m => m.LastModify()).ExecuteCommandAsync();
        ...
        public async Task Delete(string id)
        {
            ...
            if (entity != null)
            {
                await _repository.Context.Deleteable<OrderEntryEntity>(x => x.OrderId == id).ExecuteCommandAsync();
                await _repository.Context.Deleteable<OrderReceivableEntity>(x => x.OrderId == id).ExecuteCommandAsync();
                await _repository.Context.Updateable(entity).CallEntityMethod(m => m.Delete()).UpdateColumns(it => new { it.DeleteMark, it.DeleteTime, it.DeleteUserId }).ExecuteCommandHasChangeAsync();
        ...
        private async Task FlowSubmit(string id, FlowEngineEntity flowEngineEntity)
        {
            ...
            {
                _db.BeginTran();
                var orderEntity = await _repository.FirstOrDefaultAsync(x => x.Id == id && x.DeleteMark == null);
                #region 业务信息
                orderEntity.CurrentState = FlowTaskStatusEnum.Handle.ParseToInt();
                ...
                {
                    await _flowTaskManager.Submit(string.Empty, flowEngineEntity.Id, id, _userManager.User.RealName + "的订单示例", 1, orderEntity.OrderCode, orderEntity.Adapt<OrderInfo>(), 0);
                }
                else
                {
                    await _flowTaskManager.Submit(id, flowEngineEntity.Id, id, _userManager.User.RealName + "的订单示例", 1, orderEntity.OrderCode, orderEntity.Adapt<OrderInfo>(), 0);
                }
                #endregion
                _db.CommitTran();
            }
