...
using QT.JXC.Entitys.Dto.DocumentPreview;
...
public class DocumentPreview : IDynamicApiController, ITransient
{
    ...
    public DocumentPreview(IFileManager fileManager, IOptions<AppOptions> appOptions)
    {
    ...
    public async Task<dynamic> DocumentPreview_Api(string fileId, [FromQuery] DocumentPreviewPreviewInput input)
    {
        ...
        {
            string domain = App.GetConfig<AppOptions>("QT_App", true).Domain;
            string yozoUrl = App.GetConfig<AppOptions>("QT_App", true).YOZO.Domain;
            string yozoKey = App.GetConfig<AppOptions>("QT_App", true).YOZO.domainKey;
