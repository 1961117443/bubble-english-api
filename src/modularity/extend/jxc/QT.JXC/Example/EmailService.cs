...
using QT.JXC.Entitys.Dto.Email;
...
public class EmailService : IDynamicApiController, ITransient
{
    ...
    {
        var output = (await GetInfo(id)).ToObject<EmailInfoOutput>();
        output.bodyText = HttpUtility.HtmlDecode(output.bodyText);
        return output;
    }
    ...
    public async Task Delete(string id)
    {
        try
        {
            ...
            var isOk = false;
            if (entity is EmailReceiveEntity)
                isOk = await _repository.Context.Updateable((EmailReceiveEntity)entity).CallEntityMethod(m => m.Delete()).UpdateColumns(it => new { it.DeleteMark, it.DeleteTime, it.DeleteUserId }).ExecuteCommandHasChangeAsync();
            else
                isOk = await _repository.Context.Updateable((EmailSendEntity)entity).CallEntityMethod(m => m.Delete()).UpdateColumns(it => new { it.DeleteMark, it.DeleteTime, it.DeleteUserId }).ExecuteCommandHasChangeAsync();
            if (!isOk)
    ...
    {
        var isOk = await _repository.Context.Updateable<EmailReceiveEntity>().SetColumns(it => new EmailReceiveEntity()
        {
            Read = 1,
    ...
    public async Task<dynamic> Receive()
    {
        ...
        if (mailConfig != null)
        {
            ...
            if (MailUtil.CheckConnected(mailAccount))
            {
                ...
                var receiveCount = await _repository.Context.Queryable<EmailReceiveEntity>().CountAsync(x => x.MAccount == mailConfig.Account && SqlFunc.Between(x.CreatorTime, startTime, endTime));
                ...
                if (entitys.Count > 0)
                {
                    await _repository.Context.Insertable(entitys).CallEntityMethod(m => m.Creator()).ExecuteCommandAsync();
                }
    ...
    public async Task SaveDraft([FromBody] EmailActionsSaveDraftInput input)
    {
        ...
        entity.Sender = App.GetConfig<AppOptions>("QT_App", true).ErrorReportTo;
        var isOk = 0;
        entity.State = -1;
        if (entity.Id.IsEmpty())
        {
            isOk = await _repository.Context.Insertable(entity).CallEntityMethod(m => m.Creator()).ExecuteCommandAsync();
        }
        else
        {
            isOk = await _repository.Context.Updateable(entity).IgnoreColumns(ignoreAllNullColumns: true).CallEntityMethod(m => m.LastModify()).ExecuteCommandAsync();
        }
    ...
    public async Task SaveConfig([FromBody] EmailConfigUpInput input)
    {
        ...
        {
            entity.Id = data.Id;
            isOk = await _repository.Context.Updateable(entity).IgnoreColumns(ignoreAllNullColumns: true).ExecuteCommandAsync();
        }
        if (isOk < 1)
            throw Oops.Oh(ErrorCode.COM1008);
    }
    ...
    {
        return await _repository.Context.Queryable<EmailConfigEntity>().FirstAsync(x => x.CreatorUserId == _userManager.UserId);
    }
    ...
    private async Task<dynamic> GetReceiveList(EmailListQuery input)
    {
        ...
        var list = await _repository.Context.Queryable<EmailReceiveEntity>().Where(whereLambda)
    ...
    private async Task<dynamic> GetUnreadList(EmailListQuery input)
    {
        ...
        var list = await _repository.Context.Queryable<EmailReceiveEntity>().Where(whereLambda).OrderBy(x => x.Date, OrderByType.Desc).ToPagedListAsync(input.currentPage, input.pageSize);
        var pageList = new SqlSugarPagedList<EmailListOutput>()
        {
            list = list.list.Adapt<List<EmailListOutput>>(),
            pagination = list.pagination
    ...
    private async Task<dynamic> GetDraftList(EmailListQuery input)
    {
        ...
        var list = await _repository.Context.Queryable<EmailSendEntity>().Where(whereLambda).OrderBy(x => x.CreatorTime, OrderByType.Desc).ToPagedListAsync(input.currentPage, input.pageSize);
        var pageList = new SqlSugarPagedList<EmailListOutput>()
        {
            list = list.list.Adapt<List<EmailListOutput>>(),
            pagination = list.pagination
    ...
    private async Task<object> GetInfo(string id)
    {
        ...
        if (await _repository.Context.Queryable<EmailReceiveEntity>().AnyAsync(x => x.Id == id && x.DeleteMark == null))
        {
            var receiveInfo = await _repository.Context.Queryable<EmailReceiveEntity>().FirstAsync(x => x.Id == id && x.DeleteMark == null);
            receiveInfo.Read = 1;
            await _repository.Context.Updateable(receiveInfo).CallEntityMethod(m => m.LastModify()).UpdateColumns(x => new { x.LastModifyTime, x.LastModifyUserId, x.Read }).ExecuteCommandHasChangeAsync();
            entity = receiveInfo;
        }
        else
        {
            entity = await _repository.Context.Queryable<EmailReceiveEntity>().FirstAsync(x => x.Id == id && x.DeleteMark == null);
        }
        return entity;
    }
    #endregion
