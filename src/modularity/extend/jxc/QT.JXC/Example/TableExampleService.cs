...
using QT.JXC.Entitys.Dto.TableExample;
...
public class TableExampleService : IDynamicApiController, ITransient
{
    ...
    public TableExampleService(
        ISqlSugarRepository<TableExampleEntity> tableExampleRepository,
...
    /// <returns></returns>
    ...
    {
        var list = await _tableExampleRepository.Context.Queryable<TableExampleEntity, UserEntity>((a, b) => new JoinQueryInfos(JoinType.Left, a.Registrant == b.Id))
    ...
    {
        var data = (await _tableExampleRepository.Context.Queryable<ProvinceEntity>()
    ...
    {
        var data = (await _tableExampleRepository.Context.Queryable<ProvinceEntity>().Where(m => m.ParentId == id && m.DeleteMark == null).ToListAsync()).Adapt<List<TableExampleCityListOutput>>();
        return new { list = data };
    }
    ...
    {
        var dicData = await _tableExampleRepository.Context.Queryable<DictionaryDataEntity, DictionaryTypeEntity>((a, b) =>
