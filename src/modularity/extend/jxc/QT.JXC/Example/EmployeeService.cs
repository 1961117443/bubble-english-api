...
using QT.JXC.Entitys.Dto.Employee;
...
    /// </summary>
    ...
    public class EmployeeService : IDynamicApiController, ITransient
    {
        ...
        public async Task<dynamic> ImportPreview(string fileName)
        {
            try
            {
                ...
                using (var stream = (await _fileManager.DownloadFileByType(filePath, fileName))?.FileStream)
                {
                    ...
                    // 得到数据
                    var excelData = ExcelImportHelper.ToDataTable(stream, true);
        ...
        public async Task<dynamic> ExportExcelData([FromQuery] EmployeeListQuery input)
        {
            ...
            var flag = await _fileManager.UploadFileByType(stream, FileVariable.TemporaryFilePath, excelconfig.FileName);
        ...
        public async Task<dynamic> TemplateDownload()
        {
            ...
            var addFilePath = Path.Combine(FileVariable.TemplateFilePath, "职员信息模板.xlsx"); // 保存路径
            var url = string.Empty;
            if (!await _fileManager.ExistsFile(addFilePath))
            {
                var stream = await _fileManager.GetFileStream(filePath);
                var flag = await _fileManager.UploadFileByType(stream, FileVariable.TemporaryFilePath, "职员信息模板.xlsx");
                if (flag.Item1)
                {
                    url = flag.Item2;
                }
            }
            return new { name = "职员信息模板.xlsx", url = url ?? "/api/file/Download?encryption=" + DESCEncryption.Encrypt(_userManager.UserId + "|职员信息模板.xlsx", "QT") };
        }
        #endregion
