...
/// 业务实现：供货商信息.
/// </summary>
[ApiDescriptionSettings(ModuleConst.ERP,Tag = "Erp", Name = "ErpSupplier", Order = 200)]
[Route("api/Erp/[controller]")]
public class ErpSupplierService : IErpSupplierService, IDynamicApiController, ITransient
{
    /// <summary>
    /// 服务基础仓储.
    /// </summary>
    ...
    public async Task<dynamic> ImportData(IFormFile file)
    {
        ...
        try
        {
            ...
            if (excelData != null)
            {
                foreach (System.Data.DataColumn item in excelData.Columns)
                {
