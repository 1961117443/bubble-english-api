...
/// 业务实现：采购任务订单.
/// </summary>
[ApiDescriptionSettings(ModuleConst.ERP, Tag = "Erp", Name = "ErpBuyorder", Order = 200)]
...
public class ErpBuyorderService : IErpBuyorderService, IDynamicApiController, ITransient
{
    ...
    public async Task Create([FromBody] ErpBuyorderCrInput input)
    {
        ...
        foreach (var g in input.erpBuyorderdetailList.GroupBy(it => it.supplier))
        {
            ...
            if (erpBuyorderdetailEntityList != null)
            {
                ...
                await _repository.Context.Insertable(erpBuyorderdetailEntityList).ExecuteCommandAsync();
    ...
    public async Task Update(string id, [FromBody] ErpBuyorderUpInput input)
    {
        ...
        if (result.update.Any())
        {
            ...
            {
                await _repository.Context.Updateable(updateErpBuyorderdetailEntitys).ExecuteCommandAsync();
            }
        }
        ...
        if (buyorders.IsAny())
        {
            ...
            if (inrecordList.IsAny())
            {
                ...
                {
                    await _repository.Context.Updateable(updateErpInrecordEntitys).ExecuteCommandAsync();
                }
                if (updateErpOrderdetailEntitys.IsAny())
                {
                    await _repository.Context.Updateable(updateErpOrderdetailEntitys).ExecuteCommandAsync();
                }
            }
        }
    ...
    public async Task Delete(string id)
    {
        ...
        // 明细有审核也不允许删除
        if (await _repository.Context.Queryable<ErpBuyorderdetailEntity>().Where(it => it.Fid == id && it.AuditTime != null).AnyAsync())
        {
            throw Oops.Oh("明细已入库，不允许删除！");
        }
    ...
    public async Task<List<ErpProductRecommendOutput>> GetRecommendListV2([FromQuery] ErpProductRecommendInput input)
    {
        ...
        var storeList = await _repository.Context.UnionAll(q1, q2).GroupBy(x => x.mid)
            .Select(x => new ErpBuyorderRecommendDto
            {
                ...
                buyNum = SqlFunc.AggregateSum(x.buyNum)
            }).ToListAsync();
        ...
        {
            //return new List<ErpProductRecommendOutput>();  // 没有数据一样返回下单的商品
        }
        ...
        List<ErpBuyorderRecommendDto> otherStocks = new List<ErpBuyorderRecommendDto>();
        if (config.erpShareStock.IsNotEmptyOrNull())
        {
            ...
            {
                // 计算其他库存
                otherStocks = await _repository.Context.Queryable<ErpInorderEntity>()
                    .InnerJoin<ErpInrecordEntity>((a, b) => a.Id == b.InId)
                    .ClearFilter<ICompanyEntity>()
                .Where((a, b) => a.Oid != oid && companys.Contains(a.Oid) && b.Num > 0)
                .Where((a, b) => SqlFunc.Subqueryable<ErpProductmodelEntity>().Where(m => m.Id == b.Gid && productList.Contains(m.Pid)).Any())
                .Select((a, b) => new ErpBuyorderRecommendDto
                {
                    mid = b.Gid,
        ...
        var arr = orderList.Select(x => x.mid).Distinct().ToArray();
        ...
        {
            var entitys = orderList.Where(x => x.mid == itema.id).ToList();
            if (entitys.IsAny())
            {
                itema.num = entitys.Sum(a => a.needNum);
                itema.storeNum = storeList.Where(x => x.mid == itema.id).Sum(a => a.srcStoreNum);
                itema.buyNum = storeList.Where(x => x.mid == itema.id).Sum(a => a.buyNum);


                itema.remark = string.Join("+", entitys.Select(x => string.IsNullOrEmpty(x.remark) ? $"{float.Parse(x.orderNum.ToString())}" : $"{float.Parse(x.orderNum.ToString())}({x.remark})"));

                // 关联其他库存
                var os = otherStocks.FirstOrDefault(x => x.mid == itema.id);
                if (os!=null)
                {
                    itema.relationStoreNum = os.storeNum;
                }
                itema.totalStoreNum = itema.relationStoreNum + itema.storeNum;

                itema.orderDetails = string.Join(",", entitys.Select(x => x.orderId));
            }
        }
        return data;
    }
    ...
    /// <returns></returns>
    [HttpPost("recommend/detail")]
    public async Task<List<ErpBuyorderExportRecommendList>> GetRecommendDetailList([FromBody]List<string> orderDetailIds)
    {
        var orderList = await _repository.Context.Queryable<ErpOrderEntity>()
           .InnerJoin<ErpOrderdetailEntity>((a, b) => a.Id == b.Fid)
           .InnerJoin<ErpProductmodelEntity>((a, b, c) => b.Mid == c.Id)
           .Where((a, b) => orderDetailIds.Contains(b.Id))
           .Select((a, b, c) => new ErpBuyorderExportRecommendList
           {
               no = a.No,
               posttime = a.Posttime.Value.ToString("yyyy-MM-dd"),
               cidName = SqlFunc.Subqueryable<ErpCustomerEntity>().Where(ddd=>ddd.Id ==a.Cid).Select(ddd=>ddd.Name),
               mid = b.Mid,
               midName = c.Name,
    ...
    /// <param name="orderDetailIds"></param>
    /// <returns></returns>
    [HttpPost("recommend/ExportExcel")]
    public async Task<dynamic> ExportExcelRecommendDetailList([FromBody] List<string> orderDetailIds)
    {
        var orderList = await GetRecommendDetailList(orderDetailIds);

        ExcelConfig excelconfig = ExcelConfig.Default($"分量统计明细导出_{DateTime.Now.ToString("yyyyMMddHHmmss")}.xls");
        string? addPath = Path.Combine(FileVariable.TemporaryFilePath, excelconfig.FileName);

        excelconfig.ColumnModel = Common.Extension.Extensions.GetExcelColumnModels<ErpBuyorderExportRecommendList>();

        var fs = ExcelExportHelper<ErpBuyorderExportRecommendList>.ExportMemoryStream(orderList, excelconfig);
    ...
    public async Task<dynamic> ExportExcel([FromQuery] ErpBuyorderListQueryInput input, [FromServices] IDictionaryDataService dictionaryDataService)
    {
        ...
        // 计算其他库存
        var config = await _coreSysConfigService.GetConfig<SysConfigOutput>();
        List<ErpBuyorderRecommendDto> otherStocks = new List<ErpBuyorderRecommendDto>();
        if (config.erpShareStock.IsNotEmptyOrNull())
        {
            var companys = config.erpShareStock.Split(",", true);
        ...
        foreach (var item in data)
        {
            ...
            // 关联其他库存
            var os = otherStocks.FirstOrDefault(x => x.mid == item.gid);
            if (os != null)
            {
                item.relationStoreNum = os.storeNum;
            }
            item.totalStoreNum = item.relationStoreNum + (item.storeNum ?? 0);
        }
    ...
    public async Task BatchEditPriceBySupplier(string id, [FromBody] List<string> idList)
    {
        ...
        foreach (var item in ErpBuyorderdetailEntitys)
        {
            ...
            if (priceEntity != null && priceEntity.PricingType == 2 && priceEntity.Price > 0)
            {
                var amount = Math.Round(((item.InNum ?? 0) + item.TsNum) * priceEntity.Price, 2);
                if (priceEntity.Price != item.Price || amount != item.Amount)
                {
                    // 执行改价
