...
/// 业务实现：客户信息.
/// </summary>
[ApiDescriptionSettings(ModuleConst.ERP, Tag = "Erp", Name = "ErpCustomer", Order = 200)]
[Route("api/Erp/[controller]")]
public class ErpCustomerService : IErpCustomerService, IDynamicApiController, ITransient
{
    /// <summary>
    /// 服务基础仓储.
    /// </summary>
    ...
    public ErpCustomerService(
        ...
        IUsersService usersService, IFileManager fileManager, IDictionaryDataService dictionaryDataService)
    {
    ...
    public async Task<dynamic> GetList([FromQuery] ErpCustomerListQueryInput input)
    {
        var data = await _repository.Context.Queryable<ErpCustomerEntity>()
            ...
            .WhereIF(input.stop.HasValue,it=>it.Stop == input.stop)
    ...
    public async Task Create([FromBody] ErpCustomerCrInput input)
    {
        ...
        //    _db.BeginTran();
            ...
            if (erpProductpriceEntityList != null)
            {
                ...
                await _repository.Context.Insertable(erpProductpriceEntityList).ExecuteCommandAsync();
            }
    ...
    /// </summary>
    /// <returns></returns>
    [HttpGet("Actions/{id}/Check")]
    public async Task<bool> Check([FromRoute] string id)
    {
        var result = false;
        var entity = await _repository.Context.Queryable<ErpCustomerEntity>().Where(x=>x.Id == id).Select(x => new ErpCustomerEntity
        {
            Id = x.Id,
            Type = x.Type
        }).FirstAsync();
        if (entity!=null && entity.Type.IsNotEmptyOrNull())
        {
            var typeOptions = await _dictionaryDataService.GetList("QTErpCustomType");

            var tyo = typeOptions.FirstOrDefault(x => x.EnCode == entity.Type);

            if (tyo!=null && tyo.FullName == "军队")
            {
                result = true;
            }
        }
