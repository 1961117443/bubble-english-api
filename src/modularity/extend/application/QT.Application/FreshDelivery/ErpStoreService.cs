...
public class ErpStoreService : ITransient, IErpStoreService
{
    ...
    public async Task Restore(string id)
    {
        ...
        var inList = await _repository.Context.Queryable<ErpInrecordEntity>().Where(x => inids.Contains(x.Id))
    ...
    public async Task<ErpOutdetailRecordUpOutput> Reduce(ErpOutdetailRecordUpInput input)
    {
        ...
        var inList = await _repository.Context.Queryable<ErpInrecordEntity>().Where(x => records.Contains(x.Id))
            .Select(x=> new ErpInrecordEntity
            {
                Id = x.Id,
                Num = x.Num,
        ...
        {
            var updateSql = _repository.Context.Updateable<ErpInrecordEntity>(updateInrecords).UpdateColumns(x => new { x.Num }).ToSqlString();
            sqlBuilder.AppendLine(updateSql);
        ...
        {
            var insertSql = _repository.Context.Insertable<ErpOutdetailRecordEntity>(erpOutdetailRecords).ToSqlString();
