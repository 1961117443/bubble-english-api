...
using QT.Application.Entitys.Dto.FreshDelivery.CwCustomerInvoiceDetail;
...
/// 业务实现：客户发票记录.
/// </summary>
[ApiDescriptionSettings("生鲜配送", Tag = "客户发票记录", Name = "CwCustomerInvoice", Order = 200)]
    ...
    public async Task Create([FromBody] CwCustomerInvoiceCrInput input)
    {
        ...
        //    _db.BeginTran();
            ...
            if (cwCustomerInvoiceDetailEntityList != null)
            {
                ...
                await _repository.Context.Insertable<CwCustomerInvoiceDetailEntity>(cwCustomerInvoiceDetailEntityList).ExecuteCommandAsync();
            }
