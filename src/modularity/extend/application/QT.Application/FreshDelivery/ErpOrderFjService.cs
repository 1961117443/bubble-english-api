...
/// 业务实现：订单信息.
/// </summary>
[ApiDescriptionSettings("生鲜配送", Tag = "订单分拣", Name = "ErpOrderFj", Order = 200)]
...
public class ErpOrderFjService : IErpOrderFjService, IDynamicApiController, ITransient
{
    ...
    public async Task<dynamic> GetCategoryListGroupByProduct(string id, [FromQuery] ErpOrderFjListQueryInput input)
    {
        ...
        return new
        {
            pagination = new QT.Common.Filter.PageResult
            {
                pageIndex = 1,
                pageSize = data.Count,
                total = data.Count
    ...
    public async Task<dynamic> CreateOutorder([FromBody] ErpOutdetailRecordUpInput input)
    {
        ...
        if (!await _repository.Context.Queryable<ErpInrecordEntity>().Where(x=> input.records.Any(r=>r.id == x.Id) && x.Gid == orderItem.Mid).AnyAsync())
        {
    ...
    public async Task HandleSubmit(string id,[FromBody] List<ErpOrderFjWaitListOutput> input, [FromServices] IErpOrderService erpOrderService)
    {
        ...
        await _repository.Context.Insertable<ErpOrderdetailEntity>(newOrderDetailList).ExecuteCommandAsync();
        await _repository.Context.Insertable<ErpOrderRelationEntity>(erpOrderRelationEntities).ExecuteCommandAsync();
