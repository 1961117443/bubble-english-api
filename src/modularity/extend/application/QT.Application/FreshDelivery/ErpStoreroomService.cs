using QT.Application.Entitys.Dto.FreshDelivery.ErpStorearea;
...
/// 业务实现：仓库信息.
/// </summary>
[ApiDescriptionSettings("生鲜配送", Tag = "仓库信息", Name = "ErpStoreroom", Order = 200)]
[Route("api/apply/FreshDelivery/[controller]")]
public class ErpStoreroomService : IDynamicApiController, ITransient
{
    /// <summary>
    /// 服务基础仓储.
    /// </summary>
    ...
    public async Task Create([FromBody] ErpStoreroomCrInput input)
    {
        ...
        try
        {
            ...
            if(erpStoreareaEntityList != null)
            {
                ...
                await _repository.Context.Insertable<ErpStoreareaEntity>(erpStoreareaEntityList).ExecuteCommandAsync();
            }
