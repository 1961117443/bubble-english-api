...
using QT.Application.Entitys.Dto.FreshDelivery.ErpProducttype;
...
/// 业务实现：订单信息.
/// </summary>
[ApiDescriptionSettings("生鲜配送", Tag = "销售订单.管理员", Name = "ErpOrder", Order = 200)]
...
public class ErpOrderService : IErpOrderService, IDynamicApiController, ITransient
{
    ...
    {
        var typeEnum = Common.Extension.EnumExtensions.GetEnumDescDictionary(typeof(OrderStateEnum));
    ...
    public async Task OrderDetailZeroReverse(string id)
    {
        ...
        {
            //修改
            await _repository.Context.Updateable<ErpOutdetailRecordEntity>(updateList).ExecuteCommandAsync();
        }
        if (insertList.IsAny())
        {
            //新增
            await _repository.Context.Insertable<ErpOutdetailRecordEntity>(insertList).ExecuteCommandAsync();
        }
