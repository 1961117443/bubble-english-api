...
using QT.Application.Entitys.Dto.FreshDelivery.ErpOrderGj;
...
/// 业务实现：订单改价.
/// </summary>
[ApiDescriptionSettings("生鲜配送", Tag = "订单改价", Name = "ErpOrderGj", Order = 200)]
...
public class ErpOrderGjService: IDynamicApiController
{
    ...
    public async Task<int> EditPrice(string id)
    {
        ...
        if (updateList.IsAny())
        {
            await _repository.Context.Updateable<ErpOrderdetailEntity>(updateList).EnableDiffLogEvent().ExecuteCommandAsync();
        }
