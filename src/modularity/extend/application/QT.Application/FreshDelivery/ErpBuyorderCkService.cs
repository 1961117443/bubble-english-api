...
/// 业务实现：采购任务订单.
/// </summary>
[ApiDescriptionSettings("生鲜配送", Tag = "入库审核", Name = "ErpBuyorderCk", Order = 200)]
...
public class ErpBuyorderCkService : IDynamicApiController, ITransient
{
    ...
    public async Task Update(string id, [FromBody] ErpBuyorderCkUpInput input)
    {
        ...
        {
            await _repository.Context.Updateable<ErpBuyorderdetailEntity>(erpBuyorderdetailEntityList)
    ...
    public async Task Audit(string id)
    {
        ...
        if (templist.IsAny())
        {
            templist.ForEach(x => x.BuyState = "2");
    ...
    public async Task DetailAudit(string id)
    {
        ...
        // 更新出库成本
        await this.CalcOutrecordAmount(new List<string> { entity.Gid });
