...
using QT.Application.Entitys.Dto.FreshDelivery.CwReceiptDetail;
...
/// 业务实现：收款单.
/// </summary>
[ApiDescriptionSettings("生鲜配送", Tag = "收款单", Name = "CwReceipt", Order = 200)]
[Route("api/apply/FreshDelivery/[controller]")]
public class CwReceiptService : IDynamicApiController, ITransient
{
    /// <summary>
    /// 服务基础仓储.
    /// </summary>
    ...
    public async Task Create([FromBody] CwReceiptCrInput input)
    {
        ...
        //    _db.BeginTran();
            ...
            if (cwReceiptDetailEntityList != null)
            {
                ...
                await _repository.Context.Insertable<CwReceiptDetailEntity>(cwReceiptDetailEntityList).ExecuteCommandAsync();
            }
