...
using QT.Application.Entitys.Dto.FreshDelivery.ErpOrderCwdd;
...
/// 业务实现：订单信息--财务打单.
/// </summary>
[ApiDescriptionSettings("生鲜配送", Tag = "销售订单.财务打单", Name = "ErpOrderCwdd", Order = 200)]
...
public class ErpOrderCwddService: IDynamicApiController
{
    ...
    public async Task Update(string id, [FromBody] ErpOrderCwddUpInput input)
    {
        ...
        if (updateList.IsAny())
        {
            await _repository.Context.Updateable<ErpOrderdetailEntity>(updateList).ExecuteCommandAsync();
        }
    }
}
