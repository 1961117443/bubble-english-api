...
/// 业务实现：订单信息.
/// </summary>
[ApiDescriptionSettings("生鲜配送", Tag = "销售订单.仓管", Name = "ErpOrderCg", Order = 200)]
[Route("api/apply/FreshDelivery/[controller]")]
public class ErpOrderCgService : IDynamicApiController, ITransient
{
    /// <summary>
    /// 服务基础仓储.
    /// </summary>
    ...
    public async Task Update(string id, [FromBody] ErpOrderCgUpInput input)
    {
        ...
        if (erpOrderdetailList.IsAny())
        {
            ...
            await _repository.Context.Updateable<ErpOrderdetailEntity>(erpOrderdetailList).UpdateColumns(x => new { x.SalePrice, x.Amount, x.Amount1, x.PrintAlais, x.Remark,x.ProductionDate,x.Retention }).EnableDiffLogEvent().ExecuteCommandAsync();
        ...
        if (customer != null && customer.Type.IsNotEmptyOrNull())
        {
