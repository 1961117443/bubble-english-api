...
/// 业务实现：订单信息.
/// </summary>
[ApiDescriptionSettings("生鲜配送", Tag = "订单配送", Name = "ErpOrderPs", Order = 200)]
[Route("api/apply/FreshDelivery/[controller]")]
public class ErpOrderPsService :  IDynamicApiController, ITransient
{
    /// <summary>
    /// 服务基础仓储.
    /// </summary>
    ...
    public async Task<dynamic> GetList([FromQuery] ErpOrderPsListQueryInput input)
    {
        ...
        var data = await _repository.Context.Queryable<ErpOrderEntity>()
            ...
            .Where(it => it.State == OrderStateEnum.Delivery || it.State == OrderStateEnum.Outbound)
    ...
    public async Task toDelivery(string id, [FromBody] ErpOrderPsAuditInput input, [FromServices]IErpOrderService _erpOrderService)
    {
        ...
        try
        {
            ...
            // 更新订单状态
            await _erpOrderService.ProcessOrder(entity.Id, OrderStateEnum.Delivery);
            // 关闭事务
            _db.CommitTran();
        }
        catch (Exception ex)
    ...
    public async Task toReceiving(string id, [FromBody] ErpOrderPsAuditInput input, [FromServices] IErpOrderService _erpOrderService)
    {
        ...
        try
        {
            ...
            // 更新订单状态
            await _erpOrderService.ProcessOrder(entity.Id, OrderStateEnum.Receiving);
            // 关闭事务
            _db.CommitTran();
        }
        catch (Exception)
