...
using QT.VisualData.Entitys.Dto.ScreenDataSource;
using Mapster;
...
public class ScreenDataSourceService : IDynamicApiController, ITransient
{
    ...
    {
        VisualDBEntity? entity = input.Adapt<VisualDBEntity>();
        int isOk = await _visualDBRepository.Context.Insertable(entity).IgnoreColumns(ignoreNullColumn: true).CallEntityMethod(m => m.Create()).ExecuteCommandAsync();
        if (!(isOk > 0)) throw Oops.Oh(ErrorCode.COM1000);
    }
    ...
    {
        VisualDBEntity? entity = input.Adapt<VisualDBEntity>();
        int isOk = await _visualDBRepository.Context.Updateable(entity).IgnoreColumns(ignoreAllNullColumns: true).CallEntityMethod(m => m.LastModify()).ExecuteCommandAsync();
    ...
    public async Task Delete(string ids)
    {
        ...
        int isOk = await _visualDBRepository.Context.Updateable(entity).CallEntityMethod(m => m.Delete()).UpdateColumns(it => new { it.IsDeleted }).ExecuteCommandAsync();
    ...
    public async Task<dynamic> Query([FromBody] ScreenDataSourceDynamicQueryInput input)
    {
        if (!string.IsNullOrWhiteSpace(input.id))
        {
            ...
            return await _visualDBRepository.Ado.GetDataTableAsync(input.sql);
        }
