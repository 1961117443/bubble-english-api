...
using QT.SDMS.Entitys.Dto.UserMarket;
...
/// 业务实现：营销管理-用户订单（我的部门）.
/// </summary>
[ApiDescriptionSettings("售电系统", Tag = "销售订单", Name = "CrmOrder", Order = 300)]
[Route("api/sdms/user/current/order")]
public class UserMarketService : IDynamicApiController
{
    private readonly ISqlSugarRepository<OrderEntity> _repository;
    private readonly IUserManager _userManager;

    public UserMarketService(ISqlSugarRepository<OrderEntity> repository, IUserManager userManager)
    {
    ...
    [HttpGet("dashboard")]
    public async Task<UserMarketOutput> GetInfo()
    {
        var currentId = _userManager.UserId;
        UserMarketOutput UserMarket = new UserMarketOutput()
        {
            crmUserMarketers = new List<UserMarketerOutput>(),
            myOrderList = new List<UserMarketerOrderOutput>(),
        ...
        // 获取所有的订单
        var orderList = await _repository.Context.Queryable<OrderEntity>().Where(it => users.Any(x => x.Id == it.UserId)).ToListAsync();

        // 所有的订单提成
        var orderCommissionEntities = await _repository.Context.Queryable<OrderCommissionEntity>().ToListAsync();
        ...
        foreach (var item in users.Where(x => x.Sid == currentId))
        {
            ...
            UserMarketerOutput UserMarketerOutput = new UserMarketerOutput()
            {
                id = item.Id,
            ...
            var qur = from a in orderList
                      ...
                      select new UserMarketerOrderOutput
                      {
            ...
            foreach (var xitem in qur)
            {
                ...
                {
                    UserMarketerOutput.monCommission += xitem.commission;
                }
                UserMarketerOutput.commission += xitem.commission;
            }
            #endregion
            ...
            for (int i = orderList.Count - 1; i >= 0; i--)
            {
                ...
                if (order.UserId == item.Id)
                {
                    remove = true;
                    // 下级的订单
                    UserMarketerOutput.totalAmount += order.Amount;

                    // 判断是否当月
                    if (order.OrderDate >= start && order.OrderDate <= end)
                    {
                        UserMarketerOutput.monAmount += order.Amount;
                    }
                }
                ...
                if (childs.Contains(order.UserId))
                {
                    remove = true;
                    // 孙子的订单
                    UserMarketerOutput.childTotalAmount += order.Amount;

                    // 判断是否当月
                    if (order.OrderDate >= start && order.OrderDate <= end)
                    {
                        UserMarketerOutput.childMonAmount += order.Amount;
                    }
                }
            ...
            UserMarket.crmUserMarketers.Add(UserMarketerOutput);
        }


        //UserMarket.monAmount = orderList.Where(x => x.UserId == currentId && x.OrderDate >= start && x.OrderDate <= end).Sum(x => x.Amount);
        //UserMarket.totalAmount = orderList.Where(x => x.UserId == currentId).Sum(x => x.Amount);
        //UserMarket.deptMonAmount = UserMarket.UserMarketers.Sum(x=>x.monAmount + x.childMonAmount);
        //UserMarket.deptTotalAmount = UserMarket.UserMarketers.Sum(x => x.totalAmount + x.childTotalAmount);
        ...
        //UserMarket.myOrderList = q.OrderByDescending(x => x.orderDate).ToList();

        var allChilds = GetAllChild(currentId, users); // 整个部门的人，

        // panel 统计
        UserMarket.panels = new List<object>
        {
            ...
            new { title="部门当月业绩",num=(float)UserMarket.crmUserMarketers.Sum(x=>x.monAmount + x.childMonAmount)},
            ...
            new { title="部门业绩累计",num=(float)UserMarket.crmUserMarketers.Sum(x => x.totalAmount + x.childTotalAmount)},
            new { title="部门提成累计",num=(float)orderCommissionEntities.Where(x=> allChilds.Contains(x.UserId)).Sum(x=>x.Amount)}
        };

        // 统计完后，个人业绩只显示自己的订单
        UserMarket.myOrderList = q.Where(x => x.owner == currentId).OrderByDescending(x => x.orderDate).ToList();
