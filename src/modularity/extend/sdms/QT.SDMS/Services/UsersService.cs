...
///  业务实现：用户信息.
/// </summary>
[ApiDescriptionSettings("售电系统",Tag = "销售人员管理", Name = "Users", Order = 163)]
[Route("api/sdms/[controller]")]
public class UsersService : IUsersService, IDynamicApiController, ITransient
{
    ...
    /// <returns></returns>
    [HttpGet("")]
    public async Task<dynamic> GetList([FromQuery] UserListQueryInput input)
    {
        PageInputBase? pageInput = input.Adapt<PageInputBase>();

        // 营销角色
        var ysRole = await _repository.Context.Queryable<RoleEntity>().Where(x => x.EnCode == "yx").Select(x=>x.Id).FirstAsync() ?? "-1";
        ...
        if (childOrgIds.Any())
        {
            ...
            */
            ...
            var quer = _repository.Context.Queryable<UserRelationEntity, UserEntity>((a, b) => new JoinQueryInfos(JoinType.Inner, b.Id == a.UserId))
                .Where((a,b) => QTSqlFunc.FIND_IN_SET(ysRole, b.RoleId))
                .Where((a, b) => childOrgIds.Contains(a.ObjectId))
                .WhereIF(!pageInput.keyword.IsNullOrEmpty(), (a, b) => b.Account.Contains(pageInput.keyword) || b.RealName.Contains(pageInput.keyword))
                .WhereIF(input.realName.IsNotEmptyOrNull(), (a,b)=> b.RealName.Contains(input.realName))
                .WhereIF(input.mobilePhone.IsNotEmptyOrNull(), (a, b) => b.MobilePhone.Contains(input.mobilePhone))
                .Where((a, b) => b.DeleteMark == null)
                .Select((a, b) => new UserListOutput
                {
                    id = b.Id,
        ...
        else
        {
            data = await _repository.Context.Queryable<UserEntity>()
                .Where(a=> QTSqlFunc.FIND_IN_SET(ysRole,a.RoleId))
                .WhereIF(!pageInput.keyword.IsNullOrEmpty(), a => a.Account.Contains(pageInput.keyword) || a.RealName.Contains(pageInput.keyword))
                .WhereIF(input.realName.IsNotEmptyOrNull(), (a) => a.RealName.Contains(input.realName))
                .WhereIF(input.mobilePhone.IsNotEmptyOrNull(), (a) => a.MobilePhone.Contains(input.mobilePhone))
    ...
    /// <returns></returns>
    [HttpGet("tree/List")]
    public async Task<dynamic> GetTreeList([FromQuery] UserListQueryInput input)
    {
        // 营销角色
        var ysRole = await _repository.Context.Queryable<RoleEntity>().Where(x => x.EnCode == "yx").Select(x => x.Id).FirstAsync() ?? "-1";
        ...
        if (!_userManager.IsAdministrator || input.userId.IsNotEmptyOrNull())
        {
            isCurrent = true;
            var userId = input.userId ?? _userManager.UserId;
            //根据当前账号找出所有下级
            var childList = await _repository.Context.Queryable<UserEntity>().Select(x => new UserEntity
            {
                Id = x.Id,
                Sid = x.Sid
            }).ToChildListAsync(x => x.Sid, userId, false);
        ...
        var data = await _repository.Context.Queryable<UserEntity>()
                ...
                .WhereIF(isCurrent, a => childIdList.Contains(a.Id))
    ...
    public async Task<dynamic> GetInfo(string id)
    {
        ...
        SdmsUserInfoOutput? output = entity.Adapt<SdmsUserInfoOutput>();
    ...
    {
        var role = await _repository.Context.Queryable<RoleEntity>().Where(x => x.EnCode == "yx").FirstAsync();
        if (role == null)
        {
            throw Oops.Oh("请先创建编码为[yx]的销售人员角色！");
        }
    ...
    /// <returns></returns>
    [HttpPut("{id}")]
    public async Task Update(string id, [FromBody] SdmsUserUpInput input)
    {
