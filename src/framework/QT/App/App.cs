...
public static class App
{
    ...
    private static (IEnumerable<Assembly> Assemblies, IEnumerable<Assembly> ExternalAssemblies) GetAssemblies()
    {
        ...
        // 读取项目程序集或 QT 官方发布的包，或手动添加引用的dll，或配置特定的包前缀
        var scanAssemblies = dependencyContext.RuntimeLibraries
            .Where(u =>
                   (u.Type == "project" && !excludeAssemblyNames.Any(j => u.Name.EndsWith(j))) ||
                   (u.Type == "package" && (u.Name.StartsWith(nameof(QT)) || supportPackageNamePrefixs.Any(p => u.Name.StartsWith(p)))) ||
        ...
        var externalPlugins = Settings.ExternalAssemblies?.ToList() ?? new List<string>();
        var defaultPluginDirectory = "plugins"; // 默认的插件文件夹
        // 默认加载根目录下plugins文件夹的程序集
        var pluginsPath = Path.Combine(AppContext.BaseDirectory, defaultPluginDirectory);
        if (Directory.Exists(pluginsPath))
        {
            foreach (var item in Directory.GetFiles(pluginsPath))
            {
                ...
                {
                    var dllPath = Path.Combine(defaultPluginDirectory, Path.GetFileName(item));
                    if (!externalPlugins.Any(x => Path.GetFullPath(Path.Combine(AppContext.BaseDirectory, x)).Equals(item)))
                    {
                        externalPlugins.Add(dllPath);
                    }
                }
            }
        }
        if (externalPlugins != null && externalPlugins.Any())
        {
            foreach (var externalAssembly in externalPlugins)
            {
                // 加载外部程序集
