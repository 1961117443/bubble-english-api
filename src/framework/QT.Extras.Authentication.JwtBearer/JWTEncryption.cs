...
public class JWTEncryption
{
    ...
    public static bool AutoRefreshToken(AuthorizationHandlerContext context, DefaultHttpContext httpContext, long? expiredTime = null, int refreshTokenExpiredTime = 43200, string tokenPrefix = "Bearer ", long clockSkew = 5)
    {
        ...
        if (!string.IsNullOrEmpty(httpContext.Request.Query["token"].ToString()))
        {
            ...
            //验证token
            var (IsValid, Token, _) = Validate(token.Replace(tokenPrefix, ""));
    ...
    internal static JWTSettingsOptions SetDefaultJwtSettings(JWTSettingsOptions options)
    {
        ...
        {
            options.ValidAudience ??= "powerby QT";
        }
    ...
    internal static Assembly GetFrameworkContext(Assembly callAssembly)
    {
        ...
        // 获取 QT 程序集名称
        var jnpfAssemblyName = callAssembly.GetReferencedAssemblies()
                                                   .FirstOrDefault(u => u.Name == "QT")
                                                   ?? throw new InvalidOperationException("No `QT` assembly installed in the current project was detected.");

        // 加载 QT 程序集
        var jnpfAssembly = AssemblyLoadContext.Default.LoadFromAssemblyName(jnpfAssemblyName);

        // 获取 QT.App 静态类
        FrameworkApp = jnpfAssembly.GetType("QT.App");
