...
public class RandomCaptcha : DefaultCaptcha
{
    ...
    {
        // 随机验证码类型
        options.CaptchaType = CaptchaType.WORD; // captchaTypes[10]; // captchaTypes[random.Next(0, captchaTypes.Length)];
        ...
        options.ImageOption.Animation = false; //  random.Next(2) == 0; // 动静
        ...
        options.ImageOption.Width = 114;// 150; // 验证码宽度
        options.ImageOption.Height = 32; // 50; // 验证码高度
