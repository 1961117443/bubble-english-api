...
namespace QT.Extras.Thirdparty.WeChat;
...
public class WeChatUtil
{
    ...
    public string GetDepartmentList()
    {
        ...
        if (result.errcode == 0)
        {
            return JsonSerializer.Serialize(result.department);
        }
        else
        {
    ...
    public string GetDepartmentMember(long departmentId, int fetchChild = 1)
    {
        ...
        if (result.errcode == 0)
        {
            ...
            {
                return JsonSerializer.Serialize(result.userlist);
            }
            else
            {
                return null;
    ...
    /// <param name="timeOut">代理请求超时时间（毫秒）.</param>
    /// <returns></returns>
    public async Task<bool> SendText(string agentId, string content, string toUser, string toParty = null, string toTag = null, int safe = 0, int timeOut = 10000)
    {
        var result = await MassApi.SendTextAsync(accessToken, agentId, content, toUser, toParty, toTag, safe, timeOut);
        if (result.errcode == 0)
        {
            return true;
        }
